...
class RegistoerPaymentWoreda(models.TransientModel):
    _name = "register.payment.woreda"
    _description = "Register Payment woreda Wizard"
    ...
    payments = fields.Many2many('membership.payment', string='payments')
    ...
    woreda = fields.Many2one('membership.handlers.branch', string="Wereda", domain="[('parent_id', '=', name)]",
                              required=True)


    @api.model
    def default_get(self, fields):
        res = super(RegistoerPaymentWoreda, self).default_get(fields)
        ...
        for line in active_ids:
            line_2.append(line)
            id = line
            time_frame = self.env['membership.payment'].search([('id', '=', id)], limit=1)
            total = time_frame.amount + total

        res.update({
                    "payments": [(6, 0, line_2)],
                    "fiscal_year": active_fiscal_year.id,
                    "time_frame": time_frame.month.id,
                    "name": time_frame.subcity_id.id,
                    "woreda": time_frame.wereda_id.id,
                    "amount": total,
        })
        return res
    ...
    def _pull(self):
        payment = []
        if self.pull:
            time_frame = self.env['membership.payment'].search([('month', '=', self.time_frame.id)])

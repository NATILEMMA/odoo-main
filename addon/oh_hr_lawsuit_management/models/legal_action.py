...
class HrLawsuit(models.Model):
    ...
    def write(self,vals):
        rslt = super(HrLawsuit, self).write(vals)
        model = self.env['lawsuit.appointment_stack'].sudo().search([('law_suit_id', '=', self.id)])
        if bool(vals.get('next_hearing_date')):
            next_hearing_date = datetime.strptime(vals['next_hearing_date'], "%Y-%m-%d")
            for rec in self:
                hearing_date = datetime.strptime(str(datetime.today().strftime('%Y-%m-%d')), "%Y-%m-%d")
                if rec.hearing_date:
                    hearing_date = datetime.strptime(str(rec.hearing_date), "%Y-%m-%d")
                is_new_appt = bool(hearing_date < next_hearing_date)
                if is_new_appt:
                    vals.get('hearing_date', next_hearing_date)
                    rslt = super(HrLawsuit, self).write(vals)
                    model.sudo().create({
                        'law_suit_id': rec.id,
    ...
    state = fields.Selection([('draft', 'Draft'),
                             ...
                             help='Status')
    appt_count = fields.Integer(compute="_appt_count", string="# Appointemnts", help="Legal appointments", default=0, store="True")

    @api.depends('next_hearing_date')
    def _appt_count(self):
        for rec in self:
            rec.appt_count = self.env['lawsuit.appointment_stack'].sudo().search_count([('law_suit_id', '=', rec.id)])

...
class OnlineAppointment(http.Controller):
    ...
    def online_appointment_confirm(self, **post):
        ...
        if not request.env.user._is_public():
            ...
            vals.update(
                {
                    'option': option,
                    'check_in_date': start_datetime,
                    'check_out_date': (start_datetime + datetime.timedelta(minutes=round(option.duration * 60))).strftime("%Y-%m-%d %H:%M:%S"),
                }
            )
    ...
    def sync_with_gate_visitor(self, data=False):
        ...
        fo_visitor_user = request.env['fo.visitor'].sudo().search([('name', '=', partner_info['name'])], limit=1)
        reason = request.env['fo.purpose'].sudo().search([('name', 'like', data['option'].name)], limit=1)
        if not reason:
            reason = request.env['fo.purpose'].create({
        ...
        if not fo_visitor_user:
            fo_visitor_vals = {
                ...
                'phone': partner_info['email'],
                'email': partner_info['phone'],
        ...
        fo_visit_vals = {
            'visitor': fo_visitor_user.id,
            'phone': partner_info['phone'],
            'email':partner_info['email'],
            'reason': reason,
            'check_in_date': data['check_in_date'].strftime("%Y-%m-%d %H:%M:%S"),
            'visiting_person': data['appointee_id']
        }
